...
namespace HoaxGames
{
    public class FootIK : MonoBehaviour
    {
        ...
        protected List<AnimatorStateEntry> m_forceActivateCrouchingBehaviour = new ();
        ...
        protected List<AnimatorStateEntry> m_forceInvalidOnAnimatorStates = new ();
        ...
        public Transform TargetTransform { get { return m_transform; } set { m_transform = value; } }

        protected Vector3 m_prevTransformPosition;
        protected Vector3 m_prevBodyPosition;
        protected Vector3 m_bodyOffset; public Vector3 BodyOffset { get { return m_bodyOffset; } } public Vector3 FullBodyOffset { get { return m_globalCharacterYOffset * m_transform.up + m_bodyPlacementWeight * m_bodyResetLerp * (m_bodyOffset + m_bodyPositionOffset * m_transform.up) + m_crouchOffset; } }
        ...
        protected Queue<float> m_averageSlopeLeaningValuesQueue = new ();
        ...
        protected GroundedResult m_groundedResult = new (false, false, null, Vector3.zero, Vector3.zero, Vector3.zero); public GroundedResult GetGroundedResult() { return m_groundedResult; }
        ...
        protected List<KeyValuePair<int, int>> m_deactiveOnAnimatorState = new ();
        protected List<KeyValuePair<int, int>> m_activateCrouchingOnAnimatorState = new ();
        ...
        protected virtual void UpdateIK(Vector3 feetZeroPos)
        {
            ...
            m_animator.bodyPosition = bodyPosition + m_bodyPlacementWeight * m_bodyResetLerp * (m_bodyOffset + m_bodyPositionOffset * m_transform.up);
            ...
            if (m_resultLeft.primaryHitTransform != null && leftOffsetDiff.magnitude > 0.01f)
            {
                ...
                {
                    if(dotIsMovingUpwards > 0.01f) m_extrapolationLeft = m_bodyOffset.magnitude * m_ikUpwardsPullBackDownExtrapolation * leftOffsetDiff;
            ...
            if (m_resultRight.primaryHitTransform != null && rightOffsetDiff.magnitude > 0.01f)
            {
                ...
                {
                    if(dotIsMovingUpwards > 0.01f) m_extrapolationRight = m_bodyOffset.magnitude * m_ikUpwardsPullBackDownExtrapolation * rightOffsetDiff;
            ...
            if (IsCrouching())
            {
                ...
                m_crouchOffset = Vector3.Lerp(m_crouchOffset, (1 - m_crouchCorrectionTolerance) * m_bodyPlacementWeight * (crouchOffset * m_transform.up - m_bodyOffset), Time.deltaTime * m_bodyStiffness);
            }
        ...
        protected virtual void UpdateEvents(Vector3 ikLeftTargetPos,Vector3 ikRightTargetPos, Vector3 transformMovementPlanarVec)
        {
            ...
            movedLeftVec -= upwardsLeftDirVec;
            movedRightVec -= upwardsLRightDirVec;
        ...
        private IEnumerator PlayFootstepRightEventDelayed(float delay)
        {
            if (Mathf.Approximately(delay, m_prevFootstepEventDelay) == false)
            {
                m_prevFootstepEventDelay = delay;
        ...
        protected virtual void RevertUpdateIK()
        {
            ...
            // body position
            m_animator.bodyPosition += m_bodyPlacementWeight * m_bodyResetLerp * (m_bodyOffset + m_bodyPositionOffset * m_transform.up);
...
#if UNITY_EDITOR
        private readonly HashSet<Collider> m_errorLogged = new ();
#endif
        ...
        protected virtual void FindNewIKPos(IKResult ikResult, Vector3 ikPos, Vector3 ikBottomPoint, float heightOffset, float forwardBias, Vector3 forwardDir, Quaternion rotation, Vector3 origin, Vector3 rayDirection, float maxIKCorrection, float ikCorrectionRaycastMargin, float maxFootCorrection, float maxCenterOfMassCorrection, float raycastLength, float raycastWidth, int raycastLayer)
        {
            ...
            if (Physics.BoxCast(ray.origin, new Vector3(raycastWidth * 0.5f, boxCastHeight, raycastLength * 0.5f), ray.direction, out RaycastHit hit, rotation, raycastDistance, raycastLayer, m_triggerCollisionInteraction))
            {
                ...
                if (Physics.SphereCast(fixOrigin, radius, rayDirection, out RaycastHit fixHit, raycastDistance, raycastLayer, m_triggerCollisionInteraction))
                {
                ...
                if (Vector3.Dot(distVec.normalized, rayDirection) > 0)
                {
                    ...
                    if (distVec.magnitude > maxFootCorrection)
                    {
                        ...
                        surfacePoint += surfaceOffsetVec;
                        foundBottomPoint += surfaceOffsetVec;
                    }
                }
                else
                {
        ...
        protected virtual void IsValidAndGrounded(GroundedResult groundedResult, Vector3 feetZeroPos, Vector3 upVec, int collisionLayer)
        {
            ...
            if (Physics.SphereCast(origin, m_checkGroundedRadius, -upVec, out RaycastHit hit, raycastDistance, collisionLayer, m_triggerCollisionInteraction))
            {
